...
#define SUBTITLE "Contact me using this web server."
#define TITLE "Write here your <br><br> full name ,<br><br> phone number ,<br><br>class section &<br><br>class year"
#define BODY "Write your query."
#define POST_TITLE "Sent..."
#define POST_BODY "Your query has been sent to the Om Vataliya.</br>Thank you."
#define PASS_TITLE "Details"
...
String footer() { 
  return "</div><div class=q><a>&#169; All rights reserved by Om Vataliya.</a></div>";
}

String header(String t) {
  String a = String(currentSSID);
  String CSS = "article { background: black; border: 2px solid white padding: 1.3em; }" 
    "body { color: white; background-color:gray; font-family: Century Gothic, sans-serif; font-size: 18px; line-height: 24px; margin: 30; padding: 0; }"
    "div { padding: 0.5em; display:flex; flex-direction:column;}"
    "h1 { margin: 0.5em 0 0 0; padding: 0.5em; }"
    "input { width: 80vw; padding: 9px 10px; margin: 8px 0; box-sizing: border-box; border-radius: 0; border: 1px solid #555555; border-radius: 10px; }"
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>Your Name:</label>"+
    "<input type=text name=m></input>" + "</ul></div><div><form action=/post method=post><label>Your Number:</label>"+
    "<input type=number name=n></input>" + "</ul></div><div><form action=/post method=post><label>Your Section:</label>"+
    "<input type=number name=o></input>"+ "</ul></div><div><form action=/post method=post><label>Your Year:</label>"+
    "<input type=number name=p></input>"+ "</ul></div><div><form action=/post method=post><label>Your Query:</label>"+
    +"<input type=textarea rows=3 name=q></input><input type=submit value=Send></form>" + footer();
}
...
String posted() {
  ...
  String one =  "Name : ";
  String two = "Ph no : " ;
  String three ="Section : " ;
  String four ="Year : " ;
  String five = "Query : ";
  String pass1 = input("m");
  String pass2 = input("n");
  String pass3 = input("o");
  String pass4 = input("p");
  String pass5 = input("q");
  pass = "<li><b>" + one + pass1 + "<br>" + two + pass2 + "<br>" + three+ pass3 + "<br>" + four + pass4 +  "<br>" + five + pass5 + "</li></b>"; // Adding password in a ordered list.
...
String dGet() {
  return header(PASS_TITLE) + "<ol>" + allPass + "</ol><br><center><p><a style=\"color:blue\" href=/>Back to Index</a></p><p><a style=\"color:blue\" href=/clear>Clear details</a></p></center>" + footer();
}
...
String clear() {
  ...
  EEPROM.write(passEnd, '\0');
  EEPROM.commit();
  return header(CLEAR_TITLE) + "<div><p>The details list has been reseted.</div></p><center><a style=\"color:blue\" href=/>Back to Index</a></center>" + footer();
}
...
void setup() {
  ...
  webServer.on("/sSet",[]() { webServer.send(HTTP_CODE, "text/html", sSet()); });
  webServer.on("/postSSID",[]() { webServer.send(HTTP_CODE, "text/html", postedSSID()); });
  webServer.on("/dGet",[]() { webServer.send(HTTP_CODE, "text/html", dGet()); });
